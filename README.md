...
1. `scripts/weekly.mjs` 从 RSS / Atom、国内大厂技术官网、官方 GitHub 组织和 GitHub 新项目中获取最近 7 天的新内容。
...
- `data/web-sources.json`：字节跳动、火山引擎、阿里云、腾讯、美团等公开技术网站。
- `data/github-orgs.json`：字节、火山、阿里、通义千问、腾讯、京东、美团、快手和小红书的官方开源组织。

官网抓取只收录带有最近 7 天日期且标题与 AI、Agent、开源或工程实践相关的页面。GitHub 只收录本周活跃、达到最低 Star 门槛的非 Fork 项目。公众号仍通过投稿入口补充。

AI 输出要求简短的结构化 JSON；如果首次输出被截断或格式不完整，工作流会自动重试一次。最终发布前还会将 Markdown 形式的链接还原为候选中的真实 URL，并拒绝候选池以外的链接。
...
另有程序级硬约束：每个来源最多 2 篇，同一家企业最多 2 篇，论文最多 2 篇；政策、监管、法律、政府倡议和地缘政治类内容在候选阶段剔除，最终结果还会再次校验。AI 只能使用真实候选链接，不能自行编造来源。如果符合条件的内容不足，周刊可以少于 10 篇。
...
data/
  sources.json                自动信源
  web-sources.json            国内大厂公开技术站
  github-orgs.json            国内大厂官方 GitHub 组织
  archive.json                历史入选记录
